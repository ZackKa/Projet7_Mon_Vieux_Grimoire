...
const storage = multer.diskStorage({
	...
	filename: (req, file, callback) => { //On explique à multer quel nom de fichier utilisé.
		const name = file.originalname.split(' ').join('_');//On récupère le nom d'origine en changer les espaces avec des underscores.
...
module.exports = multer({storage: storage}).single('image');//On appelle multer et on lui passe l'objet storage, et c'est un fichier image unique grâce à single.